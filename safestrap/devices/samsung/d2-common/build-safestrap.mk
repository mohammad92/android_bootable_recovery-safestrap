BUILD_SAFESTRAP_CMD += && \
	...
	cp -fr $(SS_COMMON)/devices/samsung/d2-common/res/* $(PRODUCT_OUT)/install-files/etc/safestrap/res/ && \
	cp -fr $(SS_COMMON)/devices/samsung/d2-common/rootfs/* $(PRODUCT_OUT)/install-files/etc/safestrap/rootfs/ && \
	cp -fr $(SS_COMMON)/devices/samsung/d2-common/ss.config $(PRODUCT_OUT)/APP/ss.config && \
	cp -fr $(SS_COMMON)/devices/samsung/d2-common/ss.config $(PRODUCT_OUT)/install-files/etc/safestrap/ss.config && \
