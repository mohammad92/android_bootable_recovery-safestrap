...
BUILD_SAFESTRAP_CMD += && \
	cp -fr $(SS_COMMON)/devices/motorola/common-omap4/res/* $(PRODUCT_OUT)/install-files/etc/safestrap/res/ && \
	...
	cp -fr $(SS_COMMON)/devices/motorola/solana/res/* $(PRODUCT_OUT)/install-files/etc/safestrap/res/ && \
	cp -fr $(SS_COMMON)/devices/motorola/solana/ss.config $(PRODUCT_OUT)/install-files/etc/safestrap/ss.config && \
