...
INSTALL_SAFESTRAP_DEVICE_CMD := \
	...
	mkdir -p $(PRODUCT_OUT)/install-files/etc/safestrap/kexec; \
	cp $(PRODUCT_OUT)/kernel $(PRODUCT_OUT)/install-files/etc/safestrap/kexec/kernel; \
	cp device/motorola/solana/kexec/* $(PRODUCT_OUT)/install-files/etc/safestrap/kexec/; \
	cp device/motorola/omap4-common/kexec/kexec $(PRODUCT_OUT)/install-files/etc/safestrap/kexec/; \
	cp $(SS_COMMON)/devices/motorola/solana/safestrapmenu $(PRODUCT_OUT)/install-files/etc/safestrap/;
