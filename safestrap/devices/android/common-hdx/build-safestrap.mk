BUILD_SAFESTRAP_CMD += && \
	...
	cp -fr $(SS_COMMON)/devices/android/common-hdx/rootfs/* $(PRODUCT_OUT)/install-files/etc/safestrap/rootfs/ && \
	cp -fr $(SS_COMMON)/devices/android/common-hdx/ss.config $(PRODUCT_OUT)/APP/ss.config && \
	cp -fr $(SS_COMMON)/devices/android/common-hdx/ss.config $(PRODUCT_OUT)/install-files/etc/safestrap/ss.config && \
