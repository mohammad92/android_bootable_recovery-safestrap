BUILD_SAFESTRAP_CMD += && \
	...
	cp -fr $(SS_COMMON)/devices/android/jem/res/* $(PRODUCT_OUT)/install-files/etc/safestrap/res/ && \
	cp -fr $(SS_COMMON)/devices/android/jem/ss.config $(PRODUCT_OUT)/APP/ss.config && \
	cp -fr $(SS_COMMON)/devices/android/jem/ss.config $(PRODUCT_OUT)/install-files/etc/safestrap/ss.config && \
