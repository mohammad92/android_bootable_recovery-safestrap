...
readConfig() {
	...
	SS_LOC=$($BBX fgrep "SS_LOC=" $SS_CONFIG | $BBX sed 's/SS_LOC=//')
	...
	SS_COPY_SYSTEM_DATA=$($BBX fgrep "SS_COPY_SYSTEM_DATA=" $SS_CONFIG | $BBX  sed 's/SS_COPY_SYSTEM_DATA=//')
...
dumpConfig() {
	...
	$BBX echo "<1>SS_LOC=$SS_LOC" > /dev/kmsg
	...
	$BBX echo "<1>SS_COPY_SYSTEM_DATA=$SS_COPY_SYSTEM_DATA" > /dev/kmsg
